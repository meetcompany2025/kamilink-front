...
export default function FreightRequestEditPage() {
...
const removeFile = (index: number) => {
  ...
  if (id) {
    async function fetchFreight() {
      try {
        const data = await FreightRequestService.getById(id)
...
useEffect(() => {
  if (isEditMode && freightToEdit) {
    form.reset({
      originAddress: freightToEdit.originAddress,
      originCity: freightToEdit.originCity,
      originState: freightToEdit.originState,
      destinationAddress: freightToEdit.destinationAddress,
      ...
      weight: freightToEdit.weight,
      quantity: freightToEdit.quantity,
      length: freightToEdit.length,
      width: freightToEdit.width,
      height: freightToEdit.height,
...
try {
  const freightData = {
    ...values,
    id: freightReference,
    attachments: "", // ou arquivos processados
  ...
  if (isEditMode) {
    await FreightRequestService.update(freightId, freightData)
  } else {
                        ...
                        <FormControl>
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="Prdoutos Perigosos">Produtos Perigosos</SelectItem>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-sm">
                          {form.getValues().cargoType === "Carga Geral"
                            ? "Carga Geral"
              ...
              {currentStep < 3 ? (
                ...
                >
                  {isSubmitting
                    ...
                    : isEditMode ? "Atualizar Solicitação" : "Enviar Solicitação"}
                </Button>
              )}
            </div>
          )}
